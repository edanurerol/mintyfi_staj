 ...
 **Çözüm:** Python yüklendi ve PATH’e eklendi

### 2. `nvcc --version` komutu tanınmadı
**Çözüm:** CUDA Toolkit kuruldu ve `bin` klasörü PATH'e eklendi

### 3. PyTorch, GPU'yu tanımadı (CPU sürümü kuruldu)
**Çözüm:** Python 3.10 ile yeni bir `venv` oluşturuldu ve `torch+cu121` kuruldu

### 4. `git push` sırasında authentication hatası yaşandı
 **Çözüm:** GitHub için kişisel erişim token (PAT) oluşturuldu ve push başarıyla yapıldı
